...
lb = [lb;lb;lb];
ub = [ub;ub;ub];
options=optimset('LargeScale','off','Algorithm','active-set','Display','iter','TolFun',1.0000e-02,'DiffMinChange',1.0,'DiffMaxChange',5.0,'MaxIter',25);
% res = fmincon(@(x)objective(x,freesp,wait),x,[],[],[],[],lb,ub,@(x) ceq(x,freesp,wait),options);
res = fmincon(@(x)objective(x,freesp,wait),x,[],[],[],[],lb,ub,[],options);
...
function [g,h] = ceq(greenlight,freesp,wait)
    ...
    % 更新车辆数
    n_3_forecast = n_2_forecast + n_enter(1:28) - n_leave(1:28);
    g = [n_1_forecast-capacity;n_2_forecast-capacity;n_3_forecast-capacity];
    h = [];
